...
[![Build Status](https://img.shields.io/travis/proactionpro/yii2-scheduler/master.svg?style=flat-square)](https://travis-ci.org/proactionpro/yii2-scheduler)
[![Coverage Status](https://img.shields.io/scrutinizer/coverage/g/proactionpro/yii2-scheduler.svg?style=flat-square)](https://scrutinizer-ci.com/g/proactionpro/yii2-scheduler/code-structure)
[![Quality Score](https://img.shields.io/scrutinizer/g/proactionpro/yii2-scheduler.svg?style=flat-square)](https://scrutinizer-ci.com/g/proactionpro/yii2-scheduler)
...
~~~bash
$ composer require proactionpro/yii2-scheduler
~~~
...
~~~php
    ...
    'modules' => [
        'scheduler' => ['class' => 'proactionpro\scheduler\Module'],
    ],
    'components' => [
        'errorHandler' => [
            'class' => 'proactionpro\scheduler\ErrorHandler'
        ],
        'log' => [
...
~~~php
\yii\base\Event::on(
    \proactionpro\scheduler\console\SchedulerController::className(),
    \proactionpro\scheduler\events\SchedulerEvent::EVENT_AFTER_RUN,
    function ($event) {
        if (!$event->success) {
...
~~~bash
php yii migrate up --migrationPath=vendor/proactionpro/yii2-scheduler/src/migrations
~~~
...
class SchedulerController extends Controller
{
    public function actions()
    {
        return [
            'index' => [
                'class' => 'proactionpro\scheduler\actions\IndexAction',
                'view' => '@scheduler/views/index',
            ],
            'update' => [
                'class' => 'proactionpro\scheduler\actions\UpdateAction',
                'view' => '@scheduler/views/update',
            ],
            'view-log' => [
                'class' => 'proactionpro\scheduler\actions\ViewLogAction',
...
```php
$application->on(\proactionpro\scheduler\events\SchedulerEvent::EVENT_AFTER_RUN, function ($event) {
    if (!$event->success) {
        foreach($event->exceptions as $exception) {
            throw $exception;
        }
